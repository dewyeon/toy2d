...
class isdpAttnBlock(Fm.AllInOneBlock):   
    def __init__(self, dims_in, dims_c=[],
        ...
        super(isdpAttnBlock, self).__init__(dims_in, dims_c, subnet_constructor)
        
        self.config = cfg
        self.ISDP = ISDP(dims_in[0][0], cfg, n_head, is_multiHead)
    ...
    def forward(self, x, c=[], rev=False, jac=True):
        ...
        """
        if not rev:
            x_out, log_jac_det = self.ISDP(x_out, logdet=log_jac_det, reverse=False, permute=False)
        else:
            x_out, log_jac_det = self.ISDP(x_out, logdet=log_jac_det, reverse=True, permute=False)
        ...
        # if not rev:
        #     x_out, global_scaling_jac = self.ISDP(x_out, logdet=global_scaling_jac, reverse=False, permute=False)
        # else:
        #     x_out, global_scaling_jac = self.ISDP(x_out, logdet=global_scaling_jac, reverse=True, permute=False)
