...
class InvertISDP(nn.Module):
    def __init__(self, num_channels, split_dim, n_head, isMultiHead, cfg):
        ...
        self.cfg = cfg
        if self.cfg.action_type == 'toy-2d':
        ...
        # self.qkv_dim = self.d_split + self.cfg.condition_vec + 1 # [c/2 + pos_enc + L_layers + 1] 

        ### Y' ###
        self.conv2d_Wq1 = nn.Conv2d(self.qkv_dim, self.attn_dim, 1, 1, bias=False) # input channel, output channel, kernel, stride
        self.conv2d_Wk1 = nn.Conv2d(self.qkv_dim, self.attn_dim, 1, 1, bias=False) # input channel, output channel, kernel, stride
        self.conv2d_Wv1 = nn.Conv2d(self.qkv_dim, self.attn_dim, 1, 1, bias=False) # input channel, output channel, kernel, stride

        wq1 = nn.Parameter(torch.Tensor(self.attn_dim, self.qkv_dim, 1, 1))
        wk1 = nn.Parameter(torch.Tensor(self.attn_dim, self.qkv_dim, 1, 1))
        wv1 = nn.Parameter(torch.Tensor(self.attn_dim, self.qkv_dim, 1, 1))

        nn.init.xavier_uniform_(wq1)
        nn.init.xavier_uniform_(wk1)
        nn.init.xavier_uniform_(wv1)
        # nn.init.constant_(wv1, 0.)

        wv1.data = wv1.data * 0.01
        wv1.data[:, -1] = 1.0

        self.conv2d_Wq1.weight.data = wq1.to(self.cfg.device)
        self.conv2d_Wk1.weight.data = wk1.to(self.cfg.device)
        self.conv2d_Wv1.weight.data = wv1.to(self.cfg.device)
        self.s = torch.nn.Softmax(dim=-1)

        ### Y" ###
        self.conv2d_Wq2 = nn.Conv2d(self.qkv_dim, self.attn_dim, 1, 1, bias=False) 
        self.conv2d_Wk2 = nn.Conv2d(self.qkv_dim, self.attn_dim, 1, 1, bias=False)
        self.conv2d_Wv2 = nn.Conv2d(self.qkv_dim, self.attn_dim, 1, 1, bias=False)
        ...
        nn.init.xavier_uniform_(wq2)
        nn.init.xavier_uniform_(wk2)
        nn.init.xavier_uniform_(wv2)
        # ## nn.init.constant_(wv2, 0.)

        self.conv2d_Wq2.weight.data = wq2.to(self.cfg.device)
        self.conv2d_Wk2.weight.data = wk2.to(self.cfg.device)
        self.conv2d_Wv2.weight.data = wv2.to(self.cfg.device) * 0.1

        ''' A, B is matrix '''
        self.W_A = nn.Conv2d(self.attn_dim, self.d_split, 1, 1, bias=False) # [d', C/2]
        self.W_B = nn.Conv2d(self.attn_dim, self.d_split, 1, 1, bias=False) # [d', C/2]
        ...
        wa.data = wa.data.to(self.cfg.device) * 0.01
        wb.data = wb.data.to(self.cfg.device)
        # nn.init.constant_(wa, 0.)
        nn.init.xavier_uniform_(wb)
        # wb.data = wb.data * 0.01
        # wa.data += cfg.eps
        # wb.data += cfg.eps
        wa.data[:, 0] = 1.0
        ...
        self.scale1.data = self.scale1.to(self.cfg.device)
        self.scale2.data = self.scale2.to(self.cfg.device)
