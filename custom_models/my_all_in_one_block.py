...
class MyAllInOneBlock(InvertibleModule):
    ...
    def __init__(self, dims_in, dims_c=[],
        ...
        split_len1 = channels - channels // 2 # 384
        split_len2 = channels // 2
        self.splits = [split_len1, split_len2] # [384,384]
        ...
        self.global_scale = nn.Parameter(torch.ones(1, self.in_channels, *([1] * self.input_rank)) * float(global_scale)) # [1,768]
        self.global_offset = nn.Parameter(torch.zeros(1, self.in_channels, *([1] * self.input_rank))) # [1,768]

        if permute_soft:
            w = special_ortho_group.rvs(channels) # [768, 768]
        else:
    ...
    def forward(self, x, c=[], rev=False, jac=True):
        ...
        # x[0].shape [6272, 768]
        x1, x2 = torch.split(x[0], self.splits, dim=1) # x1, x2 : [6272, 384]

        if self.conditional:
            x1c = torch.cat([x1, *c], 1) # [6272, 512]
        else:
            x1c = x1

        if not rev:
            a1 = self.subnet(x1c) # [6272, 768]
            x2, j2 = self._affine(x2, a1) # x2: [6272, 384], j2: [6272]
        else:
            a1 = self.subnet(x1c)
            x2, j2 = self._affine(x2, a1, rev=True)

        log_jac_det = j2
        x_out = torch.cat((x1, x2), 1) # x_out : [6272, 768]

        if not rev:
            x_out, global_scaling_jac = self._permute(x_out, rev=False) # x_out [6272, 768], global_scaling_jac 0
        elif self.reverse_pre_permute:
