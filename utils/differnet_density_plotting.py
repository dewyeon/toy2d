...
from baselines.differnet.utils import *
...
def plot(batch_id, model, potential_or_sampling_fn, args):
    ...
    title = f'{args.dataset.title()}: {args.flow.title()} Flow, cb={c.n_coupling_blocks}'
    ...
    # save
    fname = f'{args.dataset}_{args.flow}_cb{c.n_coupling_blocks}_bs{args.batch_size}'
...
def plot_fwd_flow_density(model, ax, test_grid, n_pts, batch_size, args):
    ...
    for zz_i in zz.split(batch_size, dim=0):    
        ...
        zzk_i = model(zz_i)
        logdet_i = model.nf.jacobian(run_forward=False)
        zzk += [zzk_i]
        logdet += [logdet_i]
        
    zzk, logdet = torch.cat(zzk, 0), torch.cat(logdet, 0)
    nll = get_loss(zzk, logdet)
    q_log_prob = ((-1) * nll * zzk.shape[1] - logdet) / zzk.shape[1]
    log_prob = (-1) * nll
