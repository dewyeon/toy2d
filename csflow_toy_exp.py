...
from baselines.csflow.model import get_cs_flow_model, FeatureExtractor, nf_forward
import baselines.csflow.config as c
from utils.csflow_density_plotting import plot
...
parser.add_argument('--flow', type=str, default='csflow',
...
def parse_args(main_args=None):
    ...
    args.snap_dir += f'_csflow_seed{args.manual_seed}' + lr_schedule + '_' + dataset + f"_bs{args.batch_size}"
...
def compute_kl_pq_loss(model, data_or_sampler, beta, args):
    ...
    z, logdet = nf_forward(model, x)
    nll = get_loss(z, logdet)
    q_log_prob = (-1) * nll * z.shape[1] - logdet
