...
class MultiModal_MVTecDataset(Dataset):
	def __init__(self, transform, c, is_train=True):
		...
		self.cropsize = c.crp_size
		if is_train:
			self.n_transforms = c.n_transforms
		else:
			self.n_transforms = c.n_transforms_test
	...
	def __getitem__(self, idx):
		...
		if self.transform is not None:
			...
			for i in range(self.n_transforms):
				if self.get_fixed:
					samples.append(fixed_rotation(self, x, self.fixed_degrees[i]))
				else:
