...
def train_meta_epoch(c, epoch, loader, encoder, decoders, optimizer, pool_layers, N):
    ...
    for sub_epoch in range(c.sub_epochs):
        ...
        for i in range(I):
            ...
            if c.use_wandb:
                log = {"Train X space": log_prob.mean(), "Train Z space": (log_prob - log_jac_det).mean(), "Tr_loss": loss.mean().item(), "lr": lr}
                wandb.log(log)
        #
        mean_train_loss = train_loss / train_count
        if c.verbose:
            print('Epoch: {:d}.{:d} \t train loss: {:.4f}, lr={:.6f}'.format(epoch, sub_epoch, mean_train_loss, lr))
        result_train_loss += mean_train_loss

    result_tr_loss = result_train_loss/ c.sub_epochs
    return result_tr_loss
    #
...
def test_meta_epoch(c, epoch, loader, encoder, decoders, pool_layers, N):
        ...
        for i, (image, label, mask) in enumerate(tqdm(loader, disable=c.hide_tqdm_bar)):
            ...
            for l, layer in enumerate(pool_layers):
                ...
                for f in range(FIB):
                    ...
                    if c.use_wandb:
                        log = {"Test X space": log_prob.mean(), "Test Z space": (log_prob - log_jac_det).mean(), "Test_loss": loss.mean().item()}
                        wandb.log(log)
    #
    ...
    return height, width, image_list, test_dist, gt_label_list, gt_mask_list, result_test_loss
...
def train(c):
    ...
    elif c.dataset == 'multi_mvtec':
        train_dataset = MultiModal_MVTecDataset(c, is_train=True)
        test_dataset  = MultiModal_MVTecDataset(c, is_train=False)
    else:
    ...
    # stats
    ori_det_roc_obs = Score_Observer('ori_DET_AUROC')
    ori_seg_roc_obs = Score_Observer('ori_SEG_AUROC')
    p_det_roc_obs = Score_Observer('p_DET_AUROC')
    p_seg_roc_obs = Score_Observer('p_SEG_AUROC')
    ...
    if c.use_wandb:
        wandb.init(project=c.wandb_project, name=c.wandb_table)
    ...
    for epoch in range(c.meta_epochs):
        if c.action_type == 'norm-test' and c.checkpoint:
            ...
            result_train_loss = train_meta_epoch(c, epoch, train_loader, encoder, decoders, optimizer, pool_layers, N)
        else:
        ...
        height, width, test_image_list, test_dist, gt_label_list, gt_mask_list, result_test_loss = test_meta_epoch(
            c, epoch, test_loader, encoder, decoders, pool_layers, N)
        ...
        _ = ori_det_roc_obs.update(100.0*det_roc_auc, epoch)
        ...
        save_best_seg_weights = ori_seg_roc_obs.update(100.0*seg_roc_auc, epoch)
        
        result_log = {'train_loss':result_train_loss, 'test_loss': result_test_loss, 
                    'ori_DEC_AUROC max':ori_det_roc_obs.max_score, 'ori_DEC_AUROC last':ori_det_roc_obs.last,
                    'ori_SEG_AUROC max':ori_seg_roc_obs.max_score, 'ori_SEG_AUROC last':ori_seg_roc_obs.last,
                    }
        if c.use_wandb:
            wandb.log(result_log)
        
        # elif c.scoring == 'patchwise':
        for l, p in enumerate(pool_layers):
            # import pdb;pdb.set_trace()
            test_norm = torch.tensor(test_dist[l], dtype=torch.double) # (132x14x14)
            test_norm = rearrange(test_norm, '(B H W) -> B (H W)',H=height[l],W=width[l]) # (132, 14x14)
            test_norm_max = test_norm.max(dim=0).values.repeat(int(test_norm.shape[0]),1) # (132)
            test_norm = test_norm-test_norm_max
        ...
        gt_label = np.asarray(gt_label_list, dtype=np.bool)
        det_roc_auc = roc_auc_score(gt_label, score_label)
        _ = p_det_roc_obs.update(100.0*det_roc_auc, epoch)
        # calculate segmentation AUROC
        gt_mask = np.squeeze(np.asarray(gt_mask_list, dtype=np.bool), axis=1)
        seg_roc_auc = roc_auc_score(gt_mask.flatten(), super_mask.flatten())
        save_best_seg_weights = p_seg_roc_obs.update(100.0*seg_roc_auc, epoch)
        
        result_log2 = {
                    'p_DEC_AUROC max':p_det_roc_obs.max_score, 'p_DEC_AUROC last':p_det_roc_obs.last,
                    'p_SEG_AUROC max':p_seg_roc_obs.max_score, 'p_SEG_AUROC last':p_seg_roc_obs.last,
                    }
        if c.use_wandb:
            wandb.log(result_log2)

        result_list.append(result_log)
        result_list.append(result_log2)
    ...
    #
    save_results(ori_det_roc_obs, ori_seg_roc_obs, seg_pro_obs, c.model, c.class_indexes, run_date)
    save_results(p_det_roc_obs, p_seg_roc_obs, seg_pro_obs, c.model, c.class_indexes, run_date)
