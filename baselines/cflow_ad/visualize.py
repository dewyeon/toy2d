...
def export_hist(c, gts, scores, threshold):
    ...
    Y = scores.flatten()
    Y_label = gts.flatten()
    fig = plt.figure(figsize=(20*cm, 20*cm), dpi=dpi)
    ...
    fig.savefig(image_file, dpi=dpi, bbox_inches = 'tight', pad_inches = 0.0)
    plt.close()
...
def export_scores(c, test_img, scores, threshold):
    ...
    if not os.path.isdir(image_dirs):
        ...
        for i in range(num):
            ...
            #
            fig_img, ax_img = plt.subplots(2, 1, figsize=(10*cm, 20*cm))
            for ax_i in ax_img:
            ...
            #fig_img.colorbar(im1, ax=ax_img[0])
            ax_img[1].imshow(score_img)
            #fig_img.colorbar(im2, ax=ax_img[1])
            image_file = os.path.join(image_dirs, '{:08d}.png'.format(i))
            fig_img.savefig(image_file, dpi=dpi, bbox_inches = 'tight', pad_inches = 0.0)
            plt.close()
...
def export_test_images(c, test_img, gts, scores, threshold):
    ...
    if not os.path.isdir(image_dirs):
        ...
        for i in range(num):
            ...
            score_map = (255.0*scores[i]*scores_norm).astype(np.uint8)
            #
            fig_img, ax_img = plt.subplots(3, 1, figsize=(10*cm, 30*cm))
            for ax_i in ax_img:
                ax_i.axes.xaxis.set_visible(False)
                ax_i.axes.yaxis.set_visible(False)
                ax_i.spines['top'].set_visible(False)
            ...
            #fig_img.colorbar(im3, ax=ax_img[2])
            image_file = os.path.join(image_dirs, '{:08d}.png'.format(i))
            fig_img.savefig(image_file, dpi=dpi, bbox_inches = 'tight', pad_inches = 0.0)
            plt.close()